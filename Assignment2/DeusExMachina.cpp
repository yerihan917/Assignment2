...
namespace assignment2
{
	...
	unsigned int DeusExMachina::GetMoveCount(vehicleType type) const
	{
	...
	unsigned int DeusExMachina::GetSleepCount(vehicleType type) const
	{
		if (type <= vtInvalid || type >= vtCount)
		{
			return 0;
	...
	{
		//static unsigned int count = 1;	// for Test
		//printf("%02d : ", count++);		// for Test

		bool result = false;
		for (unsigned int i = 0; i < m_vehicleCount; ++i)
		{
			result = m_vehicles[i]->Move();
			//printf("%c ", (result == true ? 'y' : 'n'));	// for Test
		}
		//printf("\n");
	}
